...
/**
 * Performance-aware replacement for vanilla RandomStrollGoal on golems.
 *
 ...
 * resumes (see EntityUtilityGolem.pushEntities) only while it is actually moving.
 */
public class EntityAIGolemWander extends RandomStrollGoal {
    ...
    public boolean canUse() {
        ...
        // cheap and run BEFORE vanilla's random-position pathfinding cost.
        if (!this.golem.isPerfActive() || EntityAIGolemWander.budgetRemaining <= 0) {
            return false;
        }
        boolean execute = super.canUse();
        if (execute) {
